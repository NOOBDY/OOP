...
TEST(
    Sample,
    test_product_contains_name_and_price_should_return_correct_name_and_price) {
...
TEST(Sample,
     test_product_contains_name_length_less_than_4_should_throw_exception) {
...
TEST(Sample,
     test_cart_contains_item_should_return_correct_item_count_and_total_cost) {
    Product *products = generate_products_pointer_array(
        {"item1", "item2", "item3"}, {5, 10, 15});
...
TEST(
    Sample,
    test_cart_contains_item_should_return_correct_name_and_price_on_specific_index) {
    Product *products = generate_products_pointer_array(
        {"item1", "item2", "item3"}, {5, 10, 15});
    ShoppingCart shopping_cart = ShoppingCart(3, products);
...
TEST(
    Sample,
    test_cart_with_some_item_modified_should_return_correct_name_and_price_on_specific_index) {
    Product *products = generate_products_pointer_array(
        {"item1", "item2", "item3"}, {5, 10, 15});
    ShoppingCart shopping_cart = ShoppingCart(3, products);
...
TEST(
    Sample,
    test_cart_add_some_product_should_return_correct_item_count_and_total_cost) {
...
TEST(Sample, test_cart_set_index_out_of_range_should_throw_exception) {
    ...
    ASSERT_THROW(shopping_cart.setItemByIndex(4, Product("item4", 5000)),
                 std::string);
}
...
TEST(
    Sample,
    test_cart_put_duplicate_item_list_into_copy_constructor_should_throw_exception) {
    Product *products = generate_products_pointer_array(
        {"item1", "item1", "item3"}, {5, 5, 15});
...
TEST(Sample,
     test_two_carts_contains_the_same_products_difference_should_be_empty) {
    ShoppingCart shopping_cart;
...
TEST(Sample,
     test_two_carts_contains_products_difference_should_delete_some_stuff) {
    ShoppingCart shopping_cart;
    ShoppingCart shopping_cart2;
    ShoppingCart difference_cart;
...
TEST(Sample,
     test_two_carts_contains_the_same_products_union_should_be_the_same) {
    ShoppingCart shopping_cart;
    ShoppingCart shopping_cart2;
    ShoppingCart difference_cart;
...
TEST(
    Sample,
    test_two_carts_contains_products_union_should_append_the_missing_stuff_on_first_cart) {
    ShoppingCart shopping_cart;
    ShoppingCart shopping_cart2;
    ShoppingCart difference_cart;
...
TEST(
    Sample,
    test_two_carts_the_second_cart_assign_to_first_cart_the_item_should_be_the_same) {
    ShoppingCart shopping_cart;
    ShoppingCart shopping_cart2;
...
TEST(Sample, test_self_assign_copy_constructor_object_should_be_the_same) {
    int size = 5;
    std::vector<std::string> random_name_vector =
        generate_random_name_array(size);
    std::vector<int> random_price_vector = generate_random_price_array(size);
    Product *products = generate_products_pointer_array(random_name_vector,
                                                        random_price_vector);
    ...
    ASSERT_EQ(shopping_cart.getTotalCost(),
              calculate_total_cost_of_product(random_price_vector));
